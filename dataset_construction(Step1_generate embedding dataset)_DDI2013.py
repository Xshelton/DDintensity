...
def dump_the_model_RF_5FOLD(dataset_name,labelA,labelB,output_name):
    ...
    y_predict_prob=gsearch1.predict_proba(X_train)
    from trans_test import calculate_metric_notI
    auc,aupr=calculate_metric_notI(Y_train,y_predict_prob[:,1],1)
    #clf_final = RandomForestClassifier(n_estimators=estimators)
    #clf_final.fit(X,Y)
    print('model_training success')
    joblib.dump(gsearch1,output_name)
    print('model saving success')
    records_key={'dataset':dataset_name,'model':'RF-5fold','labelA':labelA,'labelB':labelB,'train_auc':auc,'train_aupr':aupr,'best_para':gsearch1.best_params_}
    return records_key
def load_the_model_RF_5FOLD(original_dataset_name,trans_dataset_name,trans_labelA,trans_labelB,model_name):
    from sklearn import model_selection
    from sklearn.model_selection import GridSearchCV
    from sklearn.ensemble import RandomForestClassifier
    from sklearn.preprocessing import StandardScaler
    import joblib
    print('read_orginal/transfer dataset_file')
    ...
    if df_original.iloc[0].shape!=df.iloc[0].shape:
      ...
      scaler = StandardScaler()
      df_original=df_original.drop(['{}'.format(trans_labelA),'{}'.format(trans_labelB),'label'],axis=1)
      Df_train = scaler.fit_transform(df_original)
      ...
      y_pb=clf_final.predict_proba(X_test)
    return y_predict,y_pb,Y
...
#DinoVitb16
keys='bioGPTesum'
embedding2dataset(fileA='.\\0_drug_feature_pool\\Drug_{}_label.csv'.format(keys),#lnc-palindormic_kernelpolyPCA64).csv
                                                                                  labelA='label',
                                                                                   fileB='.\\0_drug_feature_pool\\Drug_{}_label.csv'.format(keys),
                                                                                   labelB='label',
                                                                                   dataset='.\\constructed dataset\\TrainDDI2013_4128.csv',#TrainDDI2013_4128.csv
                                                                                   dataA='DBID1',
                                                                                   dataB='DBID2',
                                                                                   outputname= '.\\Embedding dataset\\0DDI2013_v{}_d{}_frs_0_rs0.csv'.format(keys,keys),
                                                                                   partion=0.01,
                                                                                   randomseed=0,
